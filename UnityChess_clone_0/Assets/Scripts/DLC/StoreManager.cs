...
public class StoreManager : MonoBehaviour
{
    ...
    public void InitializeStoreAfterLogin()
    {
        ...
        {
            UnityEngine.Debug.LogWarning("[StoreManager] Cannot initialize store — CurrentUserId is empty.");
        }
    }
    ...
    private void LoadCredits(System.Action callback = null)
    {
        if (string.IsNullOrEmpty(UserSession.CurrentUserId))
        {
            UnityEngine.Debug.LogWarning("[CREDITS] Cannot load credits — CurrentUserId is empty.");
            return;
        }
    ...
    private void LoadOwnedSkins(System.Action callback)
    {
        if (string.IsNullOrEmpty(UserSession.CurrentUserId))
        {
            UnityEngine.Debug.LogWarning("[FIRESTORE] Cannot load owned skins — CurrentUserId is empty.");
            callback?.Invoke();
            return;
        }
